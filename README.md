...
#### 🍊 **PROJECTS**
- [Ⅰ. (표현의 자유를 보장하는) **비속어 탐지 모델**](Ⅰ-표현의-자유를-보장하는-비속어-탐지-모델)
- [Ⅱ. (한/영 의료 용어를 인식하는) **의료 분야 검색 모델**](Ⅱ-한영-의료-용어를-인식하는-의료-분야-검색-모델)
- [Ⅲ. (다양한 언어를 처리하는) **금융 분야 문장 검색 모델**](Ⅲ-다양한-언어를-처리하는-금융-분야-문장-검색-모델)
- [Ⅳ. (RAG 파이프라인을 이용한) **검색 기반 한국어 LLM**](Ⅳ-rag-파이프라인을-이용한-검색-기반-한국어-llm)
- [Ⅴ. (교통사고 과실을 계산하는) **웹기반 한국어 LLM 개발 참여**](Ⅴ-교통사고-과실을-계산하는-웹기반-한국어-llm-개발-참여)       
- [Ⅵ. (4가지 방법론을 통한) **뉴스 기사 토픽 분류 모델**](Ⅵ-4가지-방법론을-통한-뉴스-기사-토픽-분류-모델) 

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;                

## Ⅰ. (표현의 자유를 보장하는) 비속어 탐지 모델
> 개인 프로젝트      
> 언어: Python         
...
**2. 한·영 의료 용어 정렬 (Post-training)**
- 한국보건의료용어표준(KOSTOM)은 한·영 의료 용어 사전으로, 각 질병마다 고유한 코드를 부여하고 있습니다.
- 동일한 코드를 지니는 용어 간의 유사도를 높임으로써 모델이 한·영 동의어를 인식할 수 있도록 했습니다. 
- Sap-BERT(Liu et al., 2020) Multi-Similarity Loss 를 이용해 동일한
  
**3. 검색 모델 학습 (Fine-tuning)**
- 
-
-
...
## Ⅳ. (RAG 파이프라인을 이용한) 검색 기반 한국어 LLM  
> 개인 프로젝트         
> 언어: Python                
> 기술 스택: Pytorch, Django            
> 모델 코드:                 
...
## Ⅴ. (교통사고 과실을 계산하는) 웹기반 한국어 LLM 개발 참여
> 팀 프로젝트     
